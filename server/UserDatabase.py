import os
from server.Configuration import CERTIFICATE_DIRECTORY
from shared.Certificate import Certificate

__author__ = 'iravid'

_database = {}
_otpsecrets = {}
_initialized = False

_servPrivKey = '-----BEGIN RSA PRIVATE KEY-----\\n-----END RSA PRIVATE KEY-----'
_servPubKey = '-----BEGIN PUBLIC KEY-----\\nlQIDAQAB\n-----END PUBLIC KEY-----'

def getByUserId(userId):
    return _database.get(userId, None)

def getUsers():
    return _database.keys()

def addCertificate(cert):
    _database[cert.userId] = cert

def getOtpSecret(cert):
    return _otpsecrets.get(cert.userId)

def addOtpSecret(cert, secret):
    _otpsecrets[cert.userId] = secret
    saveOtpSecrets()

def saveOtpSecrets():
    secrets = open(os.path.join(CERTIFICATE_DIRECTORY, "otp_secrets"), "a")

    for userId, secret in _otpsecrets.items():
        secrets.write("%s,%s\n" % (userId, secret))

    secrets.close()

def getNextId():
    return max(getUsers()) + 1

def _initialize_certificates():
    global _initialized
    if _initialized:
        return

    def walkFunc(arg, dirname, fnames):
        for fname in fnames:
            if not fname.endswith("cert"):
                continue

            cert = Certificate.deserialize(open(os.path.join(dirname, fname), "rb").read())
            addCertificate(cert)

    os.path.walk(CERTIFICATE_DIRECTORY, walkFunc, None)

def _initialize_otp_secrets():
    secrets = open(os.path.join(CERTIFICATE_DIRECTORY, "otp_secrets"))

    for line in secrets:
        if line.strip():
            userId, secret = line.strip().split(",")
            _otpsecrets[int(userId)] = secret.strip()

    _initialized = True

_initialize_certificates()
_initialize_otp_secrets()

